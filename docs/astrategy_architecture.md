...
AStrategy 是基于 MiroFish 群体智能预测引擎构建的 **A股多策略量化研究平台**。实现了 10 个独立交易策略 (S01-S10)，涵盖：
- 知识图谱传导 (供应链、事件传播)
...
│   └── s10_sentiment_simulation.py   # 情绪模拟 (LLM)
│
...
| **LLM辅助** | S04 财报超预期, S06 公告情绪, S08 行业轮动, S09 景气传导, S10 情绪模拟 | ✅ 需 DeepSeek API Key |
...
- **快速模式 (--quick)**: 30只沪深300代表性股票
- **完整模式**: 动态获取沪深300全部300只成分股
  - 主源: `ak.index_stock_cons_csindex(symbol="000300")`
  - 后备: 200+ 只硬编码股票列表
...
9. **快速回测完成 (2026-03-16)** — 30只股票, 8策略全部通过, 详见 `astrategy/.data/backtest_report.md`
...
10. **完整沪深300回测** — 用 300 只股票运行全部策略, 生成完整评估
11. **策略参数优化** — 根据回测结论调整策略参数
12. **多日历史回测** — 用过去 N 天的数据分别运行策略, 验证时间稳定性
13. **图谱策略 (S01/S03)** — 需配置 Zep Cloud 或改用本地图谱
14. **实盘/模拟盘对接** — 通过 Freqtrade Bridge 接入交易系统
15. **前端可视化** — 回测结果/信号分布/策略对比 Dashboard
