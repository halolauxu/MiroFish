...
def fetch_csi300_codes() -> list[str]:
    ...
    try:
        ...
        logger.info("从中证指数获取沪深300成分股: %d 只", len(codes))
        return codes
    except Exception as exc:
        logger.warning("获取沪深300成分股失败: %s, 使用预置列表", exc)
        return list(CSI300_FALLBACK)

# 沪深300预置列表（API失败时回退）
CSI300_FALLBACK: list[str] = [
...
STRATEGIES_WITH_LLM = ["S04", "S06", "S08", "S09", "S10"]
STRATEGIES_NEED_GRAPH = ["S01", "S03"]
ALL_STRATEGY_IDS = [f"S{i:02d}" for i in range(1, 11)]
...
class BacktestEngine:
    ...
    def generate_report(self) -> str:
        ...
        lines = [
            ...
            f"**股票池**: {len(self.stock_codes)} 只股票（沪深300）",
...
def main():
    ...
    if args.quick:
        ...
        stock_codes = fetch_csi300_codes()
